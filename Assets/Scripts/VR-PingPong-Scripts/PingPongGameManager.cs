...
public class PingPongGameManager : MonoBehaviourPunCallbacks
{
    ...
    // Runtime references (set by EnhancedRoomManager)
    ...
    /// <summary>
    /// Called by EnhancedRoomManager to set up the required prefab references
    /// </summary>
    public void SetupReferences(GameObject player1Paddle, GameObject player2Paddle, GameObject ball)
    {
        player1PaddlePrefab = player1Paddle;
        player2PaddlePrefab = player2Paddle;
        ballPrefab = ball;

        SampleController.Log("Ping Pong references setup complete");
    }

    /// <summary>
    /// Sets the table anchor that will be used as the game's reference point
    /// </summary>
    public void SetTableAnchor(SharedAnchor anchor)
    {
        ...
        // If both players are here and we have the table anchor, we can start the game
    ...
    {
        // Wait a few seconds for everything to sync up
    ...
    private void PauseGame()
    {
        ...
        if (currentBall && PhotonNetwork.IsMasterClient)
        {
    ...
    {
        if (!PhotonNetwork.IsMasterClient || ballPrefab == null) return;

        // Calculate ball position based on who's serving
        Vector3 spawnPos = ballSpawnPoint ? ballSpawnPoint.position :
            new Vector3(0, 1.2f, servingPlayer == 1 ? -1.0f : 1.0f); // Default position if no spawn point
